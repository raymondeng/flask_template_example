...
- 🎯 Entry points to execute your program using `python -m <flask_template_example>` or `$ flask_template_example` with basic CLI argument parsing.
...
---
# flask_template_example Flask Application

Awesome flask_template_example created by raymondeng
...
```bash
git clone https://github.com/raymondeng/flask_template_example flask_template_example
cd flask_template_example
make install
```
...
```bash
pip install flask_template_example
```
...
```bash
$ python -m flask_template_example
```
...
```bash
flask_template_example create-db   # run once
flask_template_example populate-db  # run once (optional)
flask_template_example add-user -u admin -p 1234  # ads a user
flask_template_example run
```
